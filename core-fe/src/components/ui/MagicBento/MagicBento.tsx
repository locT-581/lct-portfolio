...
const DEFAULT_GLOW_COLOR = "232, 90, 12";
...
export const BentoCardGrid = ({
  ...
  <div
    className={`bento-section w-full select-none relative ${className}`}
  ...
  return (
    <>
      <style>
        {`
          .bento-section {
            ...
            --border-color: var(--stroke, #242424);
            --background-dark: var(--bg-base-2, #1a1a1b);
            --white: hsl(0, 0%, 100%);
            --purple-primary: rgba(${glowColor}, 1);
            --purple-glow: rgba(${glowColor}, 0.15);
            --purple-border: rgba(${glowColor}, 0.7);
          }
          ...
          .card--border-glow:hover {
            box-shadow: 0 8px 30px rgba(0, 0, 0, 0.18), 0 0 25px rgba(${glowColor}, 0.15);
          }
      ...
      <BentoCardGrid gridRef={gridRef} className={className}>
        {children ? (
          <div className={gridClassName || "w-full"}>{children}</div>
        ) : (
          <div className="card-responsive grid gap-2">
            {cards.map((card, index) => {
              const cardKey = card.title
                ? `${card.title}-${card.label ?? index}`
                : `card-${index}`;
